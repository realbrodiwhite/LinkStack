...
<head>
  ...
  <link rel="stylesheet" href="{{ asset('assets/liinkmee/css/brands.css') }}">
  @else
...
@else
  <link rel="stylesheet" href="{{ asset('assets/liinkmee/css/share.button.css') }}">
  <link rel="stylesheet" href="{{ asset('assets/liinkmee/css/animations.css') }}">
  <link rel="stylesheet" href="{{ asset('assets/liinkmee/css/brands.css') }}">
  <link rel="stylesheet" href="{{ asset('assets/liinkmee/css/skeleton-auto.css') }}">
@endif
  ...
  <div class="container">
    <div class="row">
      <div class="column" style="margin-top: 15%">
        ...
        @if(file_exists(base_path("assets/liinkmee/images/").findFile('avatar')))
        <img alt="avatar" src="{{ asset('assets/liinkmee/images/'.findFile('avatar')) }}" width="auto" height="128px">
        @else
        <div class="logo-container fadein">
          <img src="{{ asset('assets/liinkmee/images/logo.svg') }}" alt="Logo" style="width:150px; height:150px;">
        </div>
        @endif
        ...
        @if(config('advanced-config.use_custom_buttons') == 'true')
                ...
                @foreach($array as $button)
                 ...
                 <div style="--delay: {{ $initial++ }}s" class="button-entrance"><a class="button button-{{ $button['button'] }} button button-hover icon-hover" rel="noopener noreferrer nofollow" href="{{ $button['link'] }}" @if(theme('open_links_in_same_tab') != "true")target="_blank"@endif >@if($button['icon'] == 'llc')<img alt="button-icon" class="icon hvr-icon" src="{{ asset('\/assets/liinkmee/icons\/')}}llc.svg">@else<i style="color: {{$button['icon']}}" class="icon hvr-icon fa {{$button['icon']}}"></i>@endif {{ $button['title'] }}</a></div>
                 ...
                 <div style="--delay: {{ $initial++ }}s" class="button-entrance"><a class="button button-coffee button button-hover icon-hover" rel="noopener noreferrer nofollow" href="{{ $button['link'] }}" @if(theme('open_links_in_same_tab') != "true")target="_blank"@endif ><img alt="button-icon" class="icon hvr-icon" src="@if(theme('use_custom_icons') == "true"){{ url('themes/' . $GLOBALS['themeName'] . '/extra/custom-icons')}}/coffee{{theme('custom_icon_extension')}} @else{{ asset('\/assets/liinkmee/icons\/')}}coffee.svg @endif">Buy me a Coffee</a></div>
                 ...
                 <h2>{{ $button['title'] }}</h2>
                 @else
                 <div style="--delay: {{ $initial++ }}s" class="button-entrance"><a class="button button-{{ $button['button'] }} button button-hover icon-hover" @if($button['link'] != '') href="{{ $button['link'] }}" target="_blank"@endif><img alt="button-icon" class="icon hvr-icon" src="{{ asset('\/assets/liinkmee/icons\/') . $linkName }}.svg">{{ ucfirst($linkName) }}</a></div>
                 @endif
                @endforeach
        @else
                <div style="--delay: {{ $initial++ }}s" class="button-entrance"><div class="button button-github button button-hover icon-hover"><img alt="button-icon" class="icon hvr-icon" src="{{ asset('assets/liinkmee/icons/github.svg') }}">Github</div></div>
                <div style="--delay: {{ $initial++ }}s" class="button-entrance"><div class="button button-twitter button button-hover icon-hover"><img alt="button-icon" class="icon hvr-icon" src="{{ asset('assets/liinkmee/icons/twitter.svg') }}">Twitter</div></div>
                <div style="--delay: {{ $initial++ }}s" class="button-entrance"><div class="button button-instagram button button-hover icon-hover"><img alt="button-icon" class="icon hvr-icon" src="{{ asset('assets/liinkmee/icons/instagram.svg') }}">Instagram</div></div>
        @endif
