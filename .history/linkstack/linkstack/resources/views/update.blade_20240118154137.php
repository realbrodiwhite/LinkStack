...
<img class="logo-img" src="{{ asset('assets/liinkmee/images/logo.svg') }}" alt="Logo">
        ...
        <p><?php echo __('messages.Latest beta version')."= " . external_file_get_contents("https://beta.liinkmee.org/vbeta.json"); ?></p>
        ...
        &ensp;<a class="btn" href="https://liinkmee.org/update" target="_blank"><button><i class="fa-solid fa-download btn"></i> {{__('messages.Update manually')}}</button></a>&ensp;
        </div>
        @else
        @if(env('JOIN_BETA') === true)
        <p><?php echo __('messages.Latest beta version')."= " . external_file_get_contents("https://beta.liinkmee.org/vbeta.json"); ?></p>
        ...
        <p><?php  if($Vgit > $Vlocal) {echo __('messages.You need to update to the latest mainline release');} else {echo __("messages.You’re running the latest mainline release");}  ?></p>
        @else
        <a target="_blank" href="https://github.com/linkstackorg/liinkmee/releases"><code style="color:#222;transform:scale(.9);">{{$Vlocal}} -> {{$Vgit}}</code></a>
        <h4>{{__('messages.update.manually')}}</h4>
        @endif
        <br><div class="row">
...
<?php //updating on Windows ?>
        <div class="logo-container fadein">
<img class="logo-img" src="{{ asset('assets/liinkmee/images/logo-loading.svg') }}" alt="Logo">
        </div>
...
$latestversion = trim(external_file_get_contents("https://version.liinkmee.org/"));

if(env('JOIN_BETA') === true){
   $fileUrl = 'https://beta.liinkmee.org/'. $latestversion . '.zip';
} else {
   $fileUrl = 'https://update.liinkmee.org/'. $latestversion . '.zip';
}
...
<img class="logo-img" src="{{ asset('assets/liinkmee/images/logo-loading.svg') }}" alt="Logo">
        ...
        try {
        $file = external_file_get_contents('https://pre-update.liinkmee.org');
...
<img class="logo-img" src="{{ asset('assets/liinkmee/images/logo.svg') }}" alt="Logo">
        ...
        <a class="noteslink" href="https://github.com/linkstackorg/liinkmee/releases/latest" target="_blank"><i class="fa-solid fa-up-right-from-square"></i> {{__('messages.View the release notes')}}</a>
        <br>
        @endif
        <br><div class="row">
        &ensp;<a class="btn" href="{{ url('dashboard') }}"><button><i class="fa-solid fa-house-laptop btn"></i> {{__('messages.Admin Panel')}}</button></a>&ensp;
